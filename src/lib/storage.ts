...
import { migrateSlideDeviceElements } from "./device-elements";
import type { Device, ElementTransform, Orientation, ProjectState, Slide, TextElement } from "./types";
...
function migrateSlide(slide: Slide, canvasDevice: Device, orientation: Orientation): Slide {
  ...
  return migrateSlideDeviceElements(base, canvasDevice, orientation);
}
  ...
  const slidesByDevice = parsed.slidesByDevice
    ? Object.fromEntries(
        Object.entries(parsed.slidesByDevice).map(([device, slides]) => {
          const canvasDevice = device as Device;
          const orientation =
            typeof parsed.orientation === "string" ? parsed.orientation : "portrait";
          return [
            device,
            Array.isArray(slides)
              ? slides.map((slide) => migrateSlide(slide as Slide, canvasDevice, orientation))
              : [],
          ];
        }),
      )
