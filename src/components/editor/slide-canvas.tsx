...
import { effectiveBgStyle, resolveSlideBackground, shade } from "@/lib/color";
import {
  deviceElementKey,
...
function Caption({
  ...
  const labelFontSize = slide.labelStyle?.fontSize ?? defaultLabelFontSize(cW, cH);
  const labelFontWeight = slide.labelStyle?.fontWeight ?? 600;
  const labelColor = slide.labelStyle?.color ?? defaultLabelColor(theme);
  const headlineFontSize = slide.headlineStyle?.fontSize ?? defaultHeadlineFontSize(cW, cH);
  const headlineFontWeight = slide.headlineStyle?.fontWeight ?? 700;
  const headlineColor = slide.headlineStyle?.color ?? defaultHeadlineColor(theme, inverted);
  return (
    <div style={{ textAlign: align, position: "relative", width: "100%" }}>
      <EditableText
        ...
        style={{
          ...
          letterSpacing: captionUnit(cW, cH) * 0.0015,
          color: labelColor,
          textTransform: "uppercase",
          marginBottom: captionUnit(cW, cH) * 0.018,
          minHeight: captionUnit(cW, cH) * 0.03,
        }}
      />
      <EditableText
        ...
        style={{
          ...
          lineHeight: 0.96,
          letterSpacing: -captionUnit(cW, cH) * 0.001,
          color: headlineColor,
        }}
      />
    </div>
...
export function getElementTransform(
  ...
  if (isDeviceElementId(id)) {
    ...
    const deviceElement = getSlideDeviceElements(slide, device, orientation).find(
      (element) => element.id === deviceId,
    );
    return deviceElement?.transform;
  }
...
function SlideBackground({
  ...
  const { background, blobColor, useThemeBlobs } = resolveSlideBackground(slide, theme);
  const showBlobs = useThemeBlobs || effectiveBgStyle(slide) === "gradient";
  return (
    <div
      style={{
        position: "absolute",
    ...
    >
      {showBlobs && (
        <>
          <Blob cW={cW} color={blobColor} x={-15} y={-10} size={55} opacity={inverted ? 0.25 : 0.32} />
          <Blob cW={cW} color={blobColor} x={70} y={75} size={45} opacity={inverted ? 0.18 : 0.25} />
        </>
      )}
    </div>
  );
...
function SlideElements({
  ...
  function renderDeviceElement(deviceElement: (typeof deviceElements)[number]) {
    const elementId = toDeviceElementId(deviceElement.id);
    const rect = deviceElement.transform;
    const rotation = rect.rotation ?? 0;
    const zIndex = rect.zIndex ?? 3;
    const src = resolveScreenshot(deviceElement.screenshot, locale);
    return (
      <Movable
      ...
      >
        <Frame
          ...
          style={{
            width: "100%",
            height: "100%",
            ...(deviceElement.opacity !== undefined ? { opacity: deviceElement.opacity } : {}),
          }}
        />
      </Movable>
    );
  ...
  return (
    <>
      {deviceElements.map(renderDeviceElement)}
      {renderCaption()}
