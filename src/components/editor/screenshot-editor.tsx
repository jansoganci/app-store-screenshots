...
import { deviceElementKey, isBuiltInElementId, isDeviceElementId, isTextElementId, textElementKey } from "@/lib/elements";
import { syncLegacyScreenshotFields } from "@/lib/device-elements";
  ...
  const assetPaths = React.useMemo(() => {
    ...
    for (const s of allSlides) {
      const shots = [
        ...
        ...(s.deviceElements || []).map((element) => element.screenshot),
      ];
      ...
      setState((prev) => ({
        ...
        slidesByDevice: {
          ...
          [prev.device]: (prev.slidesByDevice[prev.device] || []).map((slide) => {
            ...
            if (isDeviceElementId(elementId)) {
              const deviceId = deviceElementKey(elementId);
              const current = slide.deviceElements || [];
              const nextDevices = current.map((element) =>
                element.id === deviceId ? { ...element, transform } : element,
              );
              return {
                ...slide,
    ...
    (id: string) => {
      ...
      setState((prev) => {
        ...
        const copy: Slide = {
          ...
          deviceElements: src.deviceElements?.map((element) => ({
            ...element,
            id: nid(),
            transform: { ...element.transform },
          })),
...
function slideNeedsScreenshot(device: Device, slide: Slide) {
  ...
  if (slide.layout === "no-device" || slide.layout === "feature-graphic") return false;
  const devices = slide.deviceElements || [];
  return devices.length === 0 || devices.some((element) => !element.screenshot);
}
